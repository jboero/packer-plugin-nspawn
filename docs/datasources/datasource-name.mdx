---
description: >
  The nspawn data source is used to create endless Packer plugins using
  a consistent plugin structure.
page_title: nspawn - Data Sources
nav_title: nspawn
---
...
- `mock` (string) - The name of the mock to use for the nspawn API.
...
- `mock_api_url` (string) - The nspawn API endpoint to connect to.
...
- `foo` (string) - The nspawn output foo value.
- `bar` (string) - The nspawn output bar value.
...
data "nspawn" "example" {
 ...
 source "nspawn" "example" {
   mock = data.nspawn.example.foo
 }

 build {
   sources = ["source.nspawn.example"]
 }
```
