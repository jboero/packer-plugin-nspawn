...
- A builder ([builder/nspawn](builder/nspawn))
- A provisioner ([provisioner/nspawn](provisioner/nspawn))
- A post-processor ([post-processor/nspawn](post-processor/nspawn))
- A data source ([datasource/nspawn](datasource/nspawn))
- Docs ([docs](docs))
...
Then source the built binary to the plugin path with `cp packer-plugin-nspawn ~/.packer.d/plugins/packer-plugin-nspawn`
...
This nspawn configures a [manually triggered plugin test action](/.github/workflows/test-plugin-example.yml).
...
To include plugin docs on Packer.io a global pre-hook has been added to the main nspawn .goreleaser.yml file, that if uncommented will generate and include a docs.zip file as part of the plugin release.
...
```json
{
   "title": "nspawn",
   "path": "nspawn",
   "repo": "hashicorp/packer-plugin-nspawn",
   "version": "latest",
   "sourceBranch": "main"
 }
```
...
This nspawn template is compatible with Packer >= v1.7.0
